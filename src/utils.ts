...
export function isEstablishmentOnTrial(user: User): boolean {
  ...
  const estTier = String(user.estVipTier ?? '').toLowerCase();
  const vipTier = String(user.vipTier ?? '').toLowerCase();
  if (estTier === 'trial' || vipTier === 'trial') {
    if (!user.trialEndsAt) return true;
    return new Date(user.trialEndsAt) > new Date();
  }
  if (user.trialEndsAt && (!user.estVipTier || estTier === 'free' || estTier === 'trial')) {
    return new Date(user.trialEndsAt) > new Date();
  }
  return false;
}
...
export function calculateFees(freelancerFee: number, feePercent: number): { fee: number; total: number } {
  const fee = Math.round(freelancerFee * (feePercent / 100) * 100) / 100;
