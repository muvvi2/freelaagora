...
export async function loadAllData(): Promise<AppData> {
  ...
  // Mapeamento correto alinhado com os IDs reais da tabela vip_plans_freelancer (1 a 5)
  const freelancerIdMap: Record<string, number> = { 'free': 1, 'vip1': 2, 'vip2': 3, 'vip3': 4, 'vip4': 5 };
  ...
  // Mapeamento correto alinhado com os IDs reais da tabela vip_plans_establishment (3 a 10)
  const establishmentIdMap: Record<string, number> = { 'trial': 3, 'free': 4, 'vip1': 5, 'vip2': 6, 'vip3': 7, 'vip4': 8, 'vip5': 9, 'vip6': 10 };
  const estVipPlans: EstVipPlan[] = EST_VIP_PLANS.map(plan => {
    ...
    const fee = dbPlan && dbPlan.intermediation_fee_percentage != null 
      ? Number(dbPlan.intermediation_fee_percentage) 
      : plan.intermediationFee;
