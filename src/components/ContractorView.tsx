...
import { Search, SlidersHorizontal, Plus, Megaphone, Store, Users, FileText, Pencil, MapPin, Download, Navigation, Calendar, Star, DollarSign, Crown } from 'lucide-react';
...
      // Geo filter
      if (!isWithinRadius(f, origin, radiusKm)) return false;
...
    });
    return list;
  }, [data.users, origin, radiusKm, macroFilter, category, minRating, dateFilter, customDate, query, sortBy, categoryById]);
...
              <button onClick={() => { setMacroFilter('all'); setCategory('all'); }} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold transition ${macroFilter === 'all' ? 'bg-neutral-900 text-white dark:bg-neutral-100 dark:text-neutral-900' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-300'}`}>Todas</button>
              {MACRO_CATEGORIES.map((m) => (
                <button key={m.id} onClick={() => { setMacroFilter(m.id); setCategory('all'); }} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold transition ${macroFilter === m.id ? 'text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-300'}`} style={macroFilter === m.id ? { backgroundColor: m.color } : undefined}>{m.label}</button>
              ))}
            </div>

            {/* Distance slider (always visible) */}
            <div className="flex items-center gap-3 rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2 dark:border-neutral-800 dark:bg-neutral-900">
              <MapPin className="h-4 w-4 shrink-0 text-neutral-400" />
              <span className="text-xs font-semibold text-neutral-500">Distância</span>
              <input type="range" min={1} max={100} value={radiusKm} onChange={(e) => setRadiusKm(Number(e.target.value))} className="flex-1 accent-primary-500" />
              <span className="w-12 text-right text-sm font-bold text-neutral-700 dark:text-neutral-300">{radiusKm}km</span>
            </div>
...
              <p className="text-neutral-400">Nenhum profissional encontrado com esses filtros. Tente aumentar o raio ou limpar os filtros.</p>
            </div>
          )}
        </div>
